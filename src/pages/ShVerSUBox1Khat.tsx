...
import styles from './ShVerSUBox1Khat.module.css';
...
const ShVerSUBox1Khat: React.FC = () => {
  const navigate = useNavigate();
  const location = useLocation();
  const state = location.state as LocationState;

  const [activeTab, setActiveTab] = useState(location.state?.activeTab || 'Behavioural Effects');
  ...
  const handleNextClick = () => {
    ...
    if (checkedCount >= 3) {
      ...
      navigate('/ShVerSUFollowup', {
        state: {
          fromPage: 'Khat', // or the specific substance
          activeTab,
    ...
    <IonPage>
      ...
      <IonContent>
        ...
        <div className={styles.titleSection}>
          <h2 className={styles.pageTitle}>Khat</h2>
        </div>
...
export default ShVerSUBox1Khat;