...
import styles from './ShVerCADTable1Adolescents.module.css';
...
const ShVerCADTable1Adolescents: React.FC = () => {
  const navigate = useNavigate();
  const location = useLocation(); // Remove the type parameter
  const state = location.state as LocationState; // Type assert the state instead
  ...
  const [behaviouralEffects, setBehaviouralEffects] = useState([
    ...
    {
      label: 'Delay in meeting expected developmental milestones for appropriate age (eg. smiling, sitting, interacting with others, sharing attention, walking, talking, and toilet training)',
      checked: false,
    },
  ]);
  ...
  const handleNextClick = () => {
    ...
    if (checkedCount >= 3) {
      ...
      navigate('/ShVerSUFollowup', {
        state: {
          fromPage: 'adolescents', // Specific age group selected
          activeTab,
        },
      });
    } else {
      // If fewer than 3 checked in the current tab, go to the next tab
    ...
    <IonPage>
      ...
      <IonContent>
        ...
        <div className={styles.titleSection}>
          <h2 className={styles.pageTitle}>Adolescents (age 13-18)</h2>
        </div>
...
export default ShVerCADTable1Adolescents;
